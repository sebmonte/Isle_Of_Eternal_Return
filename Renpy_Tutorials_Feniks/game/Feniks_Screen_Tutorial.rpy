...
################################################
###TUTORIAL 1: DECLARING AND SHOWING A SCREEN###
################################################
...
#what a descriptions of UI elements should look like and do
...
#summary of screen tutorial
#Screens are called at the LEFTMOST indentation, outside of other blocks so not inside labels or init blocks
#Screens are declared using show screen or call screen, the latter of which requires an interaction before
#gameplay continues



################################################
###TUTORIAL 2: Images and texts###
################################################

#You can create image variables and call them later on in the screen
...
#When defining text or adding images, you can't split the text itself into a block
#So you can't do for instance:
#Text:
#"Your score", instead it must come after text as it's part of the definition of the text element


#If I wanted to add properties to image blocks I could also
#Do it like this
...
#Summary, you add to put images on the screen and define text to put text
#You can list properties for any screen element after the element itself, and create a block
#Using a colon
#You can use a conditional statement to customize how a screen element looks

################################################
###TUTORIAL 3: BASIC CONTAINERS
################################################
...
#Summary: Containers organize their children (elements in them) on the screen, and the children
#Are positioned relative to the container
#Fixed provides a space to put in elements without organizing them
#vbox stacks elements on top of each other
#hbox stacks elements left-toright
#Elements added earlier on display behind elements added later on



################################################
###TUTORIAL 3: PROPERTIES: POSITION AND ANCHOR
################################################

#An element is a thing you are moving around, a container is a space inside which you position an element
...
#To position things an exact percentage of a pixel, use absolute(pixelvalue ie. 10.5)
...
#Anchor:
#Xanchor moves the anchor on the xaxis, yanchor moves the anchor on the y-axis, and anchor
#Let's you set both at the same time much like position
#By default, the anchor is the top-left corner
#By analogy to a corkboard: anchor is where on a photo you position the pin to put the photo in
#While position is where you push the pin in
#So for instance if you want to put the *center* of an image at (200,300)
#You will need to change the anchor of the photo, since by default it's at the top left
#So if you set the anchor at 0.5, 0.5 - that will ensure the center of the photo will be pushed in
#at the specified position
screen fenik_screen2():
    add "photo":
        pos (200, 300)
        anchor (0.5, 0.5)
